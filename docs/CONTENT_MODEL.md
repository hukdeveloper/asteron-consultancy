# Content Model — Janan Consultancy
...
- `id`, `url`, `altText` (required for accessibility), `width`, `height`, `mimeType`, `licenseType` (e.g., `LICENSED_STOCK | JANAN_OWNED | CREATIVE_COMMONS | OTHER`), `attributionText` (optional, required if license requires attribution), `uploadedByUserId` (→ User), `createdAt`, `updatedAt`.
